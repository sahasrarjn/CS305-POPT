...
// #ifdef LLC_BYPASS
    // TODO should only be applicable for POPT
        if ((cache_type == IS_LLC) && ((way == LLC_WAY) || (!simulateAccesses))) { // this is a bypass that does not fill the LLC
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        /* ====================================================================== */
        // cout << "Hi!, I'm from cache.cc trying to print out test from read queue " << RQ.entry[index].app_data.info_type << endl;
        int info_type = WQ.entry[index].app_data.info_type;
        uncore.LLC.simulateAccesses = WQ.entry[index].app_data.simulateAccesses;
        //calling different functions based on info_type
        if(info_type == 6)
        {
          uncore.LLC.setDataType(
            WQ.entry[index].app_data.addr, 
            WQ.entry[index].app_data.dType,
            WQ.entry[index].app_data.numElements,
            WQ.entry[index].app_data.elemSz,
            WQ.entry[index].app_data.totalDataTypes
          );
        }
        else if (info_type == 7){
          uncore.LLC.updateRegIndex(WQ.entry[index].app_data.index);
        }
        else if(info_type == 8)
        {
          uncore.LLC.updateIrregRanges(
            WQ.entry[index].app_data.startID,
            WQ.entry[index].app_data.endID
          );
        }
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            /* ====================================================================== */
            // cout << "Hi!, I'm from cache.cc trying to print out test from read queue " << RQ.entry[index].app_data.info_type << endl;
            int info_type = RQ.entry[index].app_data.info_type;
            uncore.LLC.simulateAccesses = RQ.entry[index].app_data.simulateAccesses;

            //calling different functions based on info_type
            if(info_type == 6)
            {
              uncore.LLC.setDataType(
                RQ.entry[index].app_data.addr, 
                RQ.entry[index].app_data.dType,
                RQ.entry[index].app_data.numElements,
                RQ.entry[index].app_data.elemSz,
                RQ.entry[index].app_data.totalDataTypes
              );
            }
            else if (info_type == 7){
              uncore.LLC.updateRegIndex(RQ.entry[index].app_data.index);
            }
            else if(info_type == 8)
            {
              uncore.LLC.updateIrregRanges(
                RQ.entry[index].app_data.startID,
                RQ.entry[index].app_data.endID
              );
            }
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    block[set][way].virtual_addr = packet->virtual_addr;
    block[set][way].data = packet->data;
    block[set][way].ip = packet->ip;
    block[set][way].cpu = packet->cpu;
    block[set][way].instr_id = packet->instr_id;
    block[set][way].m_referenceCtr = 0;
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    for (uint32_t way=0; way<NUM_WAY; way++) {
        if (block[set][way].valid && (block[set][way].tag == packet->address)) {
            ...
            match_way = way;
            // keep track of no. of references between insertion & eviction
            block[set][way].m_referenceCtr++;
...
void CACHE::setDataType(intptr_t addr, int dataTypeID, int numElements, size_t elemSz, int totalDataTypes)
{ 
    assert(dataTypeID < numMainDataTypes);
    cout << "cache.cc: registerDatatype " <<(uint64_t)addr << " " <<  (uint64_t)addr%64 << " dataTYpeID: " << dataTypeID << endl;
    auto base = addr;
    auto bound = addr + (numElements * elemSz);
    cout << elemSz << " " << dataTypeID << " print from setDataType" << endl;
    if(dataTypeID == FRONTIER && popt_addrStart[dataTypeID].size() > 0){
        popt_elemSz[dataTypeID][0] = elemSz;
        popt_addrStart[dataTypeID][0] = base;
        popt_addrEnd[dataTypeID][0] = bound;
    }else{
        // IRREG / REG DATA 
        popt_elemSz[dataTypeID].push_back(elemSz);
        popt_addrStart[dataTypeID].push_back(base);
        popt_addrEnd[dataTypeID].push_back(bound);
    }
}

void CACHE::updateRegIndex(int32_t index)
{
    uncore.LLC.regIdx = index;
}

void CACHE::updateIrregRanges(int32_t startID, int32_t endID)
{
    uncore.LLC.irreg_startID = startID;
    uncore.LLC.irreg_endID   = endID;
}
