...
/* ===================================================================== */
int test = 0;
string fn_name = "champsim_tracer";
...
void MemoryRead(VOID* addr, UINT32 index, UINT32 read_size, VOID* ip)
{
    if(!tracing_on) return;
...
{   
    app_data.info_type = 6; // label to inform cache about the function to be called.
    app_data.addr = addr;
    app_data.dType = dType;
    app_data.numElements = numElements;
    app_data.elemSz = elemSz;
    app_data.totalDataTypes = totalDataTypes;
}


void updateRegIndex(int32_t index, int tid){
    cout << "updateRegIndex " << endl;
    app_data.info_type = 7;
    app_data.index = index;
}

void updateIrregRanges(int32_t startID, int32_t endID){
    cout <<"updateIrregRanges" << endl;
    app_data.info_type = 8;
    app_data.startID = startID;
    app_data.endID = endID;
}

void startLogging(){
    cout << "pin start" << endl;
    app_data.simulateAccesses = 1;  //LLC writes will be bypassed before start and after stop
}

void stopLogging(){
    cout << "pin stop" << endl;
    app_data.simulateAccesses = 0;
}
    ...
    for (UINT32 memOp = 0; memOp < memOperands; memOp++) 
    {
        if (INS_MemoryOperandIsRead(ins, memOp)) 
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)MemoryRead,
                    IARG_MEMORYOP_EA, memOp, IARG_UINT32, memOp, IARG_UINT32, read_size, IARG_INST_PTR,
                    IARG_END);
        }
...
int main(int argc, char *argv[])
{
    ...
    cout << "callbakcs registered" << endl;
