...
class CACHE : public MEMORY {
  public:
    ...
    std::vector<std::vector<intptr_t> > popt_addrStart;
    std::vector<std::vector<intptr_t> > popt_addrEnd;
    std::vector<std::vector<int> > popt_elemSz;
    ...
    int32_t irreg_startID;
    int32_t irreg_endID;

    //Data structure for POPT replacement
    pvector<uint8_t*> offset_matrix;
    pvector<int32_t>  num_epochs;
    pvector<int32_t>  num_clines;
    pvector<int32_t>  vtx_per_line;
    int init_way;

    // TODO check if needed
    const int NUM_BANKS = DRAM_BANKS;
    const int LINE_SIZE = 64;

    bool simulateAccesses;
	...
	total_miss_latency = 0;
        ...
        popt_addrStart.resize(numMainDataTypes);
        popt_addrEnd.resize(numMainDataTypes);
        popt_elemSz.resize(numMainDataTypes);

        regIdx = -1;
        irreg_startID = -1;
        irreg_endID   = -1;

        simulateAccesses = false; // do not fill LLC if set to false
        
        for (int d = 0; d < numMainDataTypes; ++d)
        {
            offset_matrix.push_back(nullptr);
            num_epochs.push_back(-1);
            num_clines.push_back(-1);
            vtx_per_line.push_back(-1);
        }
    ...
    void copyOffsetMatrix(uint8_t* A, uint8_t* B, uint32_t size);
    void setDataType(intptr_t addr, int dataTypeID, int numElements, size_t elemSz, int totalDataTypes);
    void updateRegIndex(int32_t index);
    ...
    bool check_datatype(intptr_t addr, int &dataTypeID);
    int getVertexIDX(intptr_t addr, int dTypeID);
    uint8_t find_next_reference(int irregInd, int regInd, int dTypeID);
    void check_for_tag(int set, int &evictionWay);
    void check_for_irreg_or_frontier(int set, int &evictionWay);
};
